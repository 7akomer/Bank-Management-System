...
private:
    ...
    static stLoginRegisterRecord _ConvertLoginRegisterLineToRecord(string Line, string Seperator = "#//#")
    {
        ...
        // التحقق من أن المتجه يحتوي على 4 عناصر على الأقل
        if (LoginRegisterDataLine.size() < 4)
        {
            LoginRegisterRecord.DateTime = "";
            LoginRegisterRecord.UserName = "";
    ...
    static clsBankUser _ConvertLinetoUserObject(string Line, string Seperator = "#//#")
    {
        ...
        // التحقق من أن المتجه يحتوي على 7 عناصر على الأقل
        if (vUserData.size() < 7)
        {
            // إذا كانت البيانات ناقصة، نعيد كائن فارغ
